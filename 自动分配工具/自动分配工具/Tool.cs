...
namespace auto_distribution
{
    ...
    public partial class Tool : Form
    {
        ...
        {    
            var toolGet = stuffUsingTool.First(p => p.stuff == int.Parse(StuffNo.Text));
            if (stuffAndToolOnWork.Where(p=>p.stuff==toolGet.stuff).Count()>0)
            {
            ...
            if (toolWaiting.Contains(toolGet.using_tool))
            {
                ...
                var temp = new Stuff_using_tool();
                temp.stuff = int.Parse(StuffNo.Text);
            ...
            else
            {
                ...
                for (int i = 0; i < 200; i++)
                {
                   ...
                   if(toolWaiting.Contains(toolUseList[i].tool))
                   {
                       ...
                       //添加新的工人熟练使用的工具号
                       stuffUsingTool.Insert(int.Parse(StuffNo.Text),new Stuff_using_tool { stuff = int.Parse(StuffNo.Text), using_tool = toolUseList[i].tool });
        ...
        {
            var toolNo = int.Parse(ToolNo.Text);
            var stuffNo = stuffAndToolOnWork.FirstOrDefault(p => p.using_tool == toolNo);
            if (stuffNo != null)
            {
                toolWaiting.Add(toolNo);
                stuffAndToolOnWork.Remove(stuffAndToolOnWork.First(p => p.using_tool == toolNo));
                MessageBox.Show(stuffNo.stuff + "号工人下班，" + toolNo + "号工具已归还");
            }
            else
